...
use Migratoro\Schema\Exceptions\MethodNotFound;
use Migratoro\Schema\FieldCommand;
use Migratoro\Schema\MethodCommand;
...
class ParserTest extends BaseTestCase
{
    ...
    public function parses_has_one()
    {
        ...
        $this->assertCount(2, $this->parsed->getModel('\App\Models\User')->getMethods());
        $this->assertCount(5, $this->parsed->getModel('\App\Models\User')->getFields());
        $this->assertCount(0, $this->parsed->getModel('\App\Models\User')->getCommands());
        ...
        $this->assertCount(1, $this->parsed->getModel('\App\Models\Phone')->getMethods());
        $this->assertCount(2, $this->parsed->getModel('\App\Models\Phone')->getFields());
    ...
    public function parses_has_one_2nd()
    {
        ...
        $this->assertEquals('phone', $this->findMethod('\App\Models\User', 'phone')->getName());
    ...
    public function parses_namespaces()
    {
        ...
        $this->assertEquals('app/Models/Waaa/', $this->parsed->getPathForNamespace('App\Models\Waaa'));
    ...
    public function spec_7a()
    {
        ...
        $this->assertEquals('role_user_c', $this->parsed->getModel('\App\Models\RoleUserCustom')->getTableName());
    ...
    public function spec_8b()
    {
        ...
        $this->assertEquals('users_country', $m->getName()); // $this->hasManyThrough('App\Models\Post', 'App\Models\User');
        $this->assertEquals('User', $m->getModel()->getShortName());
    ...
    public function spec_9()
    {
        ...
        $this->assertTrue($this->findMethod(
            'Post',
            'comments'
        )->isMorphMany()); // ->morphMany("App\Models\Comment", "commentable");
        $this->assertTrue($this->findMethod(
            'Video',
            'comments'
        )->isMorphMany()); // ->morphMany("App\Models\Comment", "commentable");
    ...
    public function spec_10()
    {
        ...
        $this->assertTrue($this->findMethod('Post', 'tags')->isMorphToMany()); // morphToMany('App\Models\Tag', 'taggable')
        $this->assertTrue($this->findMethod('Video', 'tags')->isMorphToMany()); // morphToMany('App\Models\Tag', 'taggable')

        $this->assertEquals('Taggable.tag()', $this->findMethod('Tag', 'posts')->inverseMethod()->humanName());

        // morphedByMany('App\Models\Post', 'taggable')
        $this->assertTrue($this->findMethod('Tag', 'posts')->isMorphedByMany());
        // morphedByMany('App\Models\Video', 'taggable')
